...
procedure TFORVAGSUP.SetaCodigo(Codigo, Empresa: Integer);
begin
...
procedure TFORVAGSUP.FormCreate(Sender: TObject);
begin
  inherited;
  vaga := -1;
  quPrincipal1 := DmDta.quSupervisorVaga;
  DmDta.dsSupervisorVaga.DataSet := quPrincipal1;
end;

procedure TFORVAGSUP.FormActivate(Sender: TObject);
begin
  inherited;
  quprincipal1.close;
...
procedure TFORVAGSUP.btNovoClick(Sender: TObject);
begin
  inherited;
  DmDta.quSupervisorVagaEmp_cod.AsInteger := strtoint(edEmpresa.Text);
  DmDta.quSupervisorVagaVag_cod.AsInteger := strtoint(edVagCod.Text);
end;

procedure TFORVAGSUP.btSalvarClick(Sender: TObject);
begin
  if quPrincipal1.State in [dsEdit, dsinsert] then
  begin
    try
      DMDta.SalvarRegistro(DmDta.quSupervisorVaga);
...
procedure TFORVAGSUP.DBEdit3DblClick(Sender: TObject);
begin
  inherited;
  MudaAba := False;
  ...
  begin
    ...
    begin
      ...
      quPrincipal1.FieldByName('emp_cod').asinteger := FOREFU.Emp2;
