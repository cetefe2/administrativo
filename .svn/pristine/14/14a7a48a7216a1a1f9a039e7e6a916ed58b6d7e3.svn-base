...
Uses TPODTE, TPOFNC, TPOINI, TPOERTCE;

{$R *.DFM}

procedure TFORESTCE.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  Case Key of
    vk_F4: Close;
...
procedure TFORESTCE.btImprimirClick(Sender: TObject);
begin
  ...
  begin
    ...
    QRTCE2 := TQRTCE2.Create(Application);
...
procedure TFORESTCE.btFecharClick(Sender: TObject);
begin
  Close;
end;

procedure TFORESTCE.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  DmDTE.quFichaTCE.Close;
  Action := caFree;
end;

procedure TFORESTCE.FormShow(Sender: TObject);
begin
...
procedure TFORESTCE.btEnterClick(Sender: TObject);
begin
  SelectNext(ActiveControl,True,True);
end;

procedure TFORESTCE.meDtIniExit(Sender: TObject);
begin
...
procedure TFORESTCE.meDtFimExit(Sender: TObject);
begin
...
procedure TFORESTCE.btExportarClick(Sender: TObject);
begin
  Screen.Cursor := crHourGlass;
  GerarExcel(DmDTE.quFichaTCE, Caption);
  Screen.Cursor := crDefault;
end;
