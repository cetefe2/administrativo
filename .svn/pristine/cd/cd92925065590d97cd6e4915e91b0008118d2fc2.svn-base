...
type
  TFORLOTE2 = class(TForm)
    ...
    QryAbertosfat_numero: TIntegerField;
    QryAbertoscontrole: TIntegerField;
    QryItensLotecontrole: TIntegerField;
    QryAbertosfbo_valor: TFloatField;
  ...
  private
    procedure AtualizarEstagiario(lote, estudante, fatura, tce_cod, controle: integer);
  public
    { Public declarations }
...
var
  PrimeiroRegistro, UltimoRegistro, lote: integer;
  posicao: tbookmark;
begin
  ...
  begin
    ...
    lote := quPrincipal1.FieldByName('lot_numero').asinteger;
    ...
    quPrincipal1.Locate('lot_numero', lote, []);
  end;
  pgPrincipal.ActivePage := tbconsulta;
end;

procedure TFORLOTE2.AtualizarEstagiario(lote, estudante, fatura, tce_cod, controle: integer);
begin
    with TQuery.Create(nil) do
    begin
      DatabaseName := DATABASE_NAME;
      Sql.Add('update  folhapgto  set lot_numero =:lote,REGISTRO=:REGISTRO where est_cod =:estudante and fat_numero=:fatura and tce_cod = :tce_cod and controle = :controle');
      parambyname('lote').asinteger := lote;
      parambyname('estudante').asinteger := estudante;
      parambyname('fatura').asinteger := fatura;
      parambyname('tce_cod').asinteger := tce_cod;
...
begin
  ...
  begin
      ...
      AtualizarEstagiario(quprincipal1.FieldByName('lot_numero').Asinteger,
                          qryabertos.FieldByName('est_cod').Asinteger,
                          quprincipal1.FieldByName('fat_numero').Asinteger,
                          qryabertos.FieldByName('tce_cod').Asinteger,
                          qryabertos.FieldByName('controle').Asinteger);
...
begin
  if quprincipal1.FieldByName('lot_sequencia').AsString = '' then
  begin
      qryabertos.DisableControls;
      qryabertos.First;
      while not qryabertos.Eof do
      begin
        {
...
begin
  ...
  Rewrite(F1);
  //  Controle
  Registro := Formatar('399', 3, false, '0'); {1 a 3 - Código do banco}
 ...
 {CÓDIGO DO CONVÊNIO = AGÊNCIA + NÚMERO CONVÊNIO + DV}
  Registro := Registro + Formatar('1284', 5, false, '0'); {53 a 57 - Código da agência} //
  ...
  Registro := Registro + Formatar('', 1); {72 - Dígito do código do convênio no banco} // VErificar com o Banco
...
begin
  ...
  begin
    ...
    begin
      DatabaseName := DATABASE_NAME;
      Sql.Add('update  folhapgto  set lot_numero = null ,id_parcela = null,REGISTRO = NULL where lot_numero=:lote and est_cod =:estudante and fat_numero=:fatura and tce_cod = :tce_cod and controle = :controle');
      ...
      parambyname('tce_cod').asinteger := qryitenslote.FieldByName('tce_cod').Asinteger;
      parambyname('controle').asinteger := qryitenslote.FieldByName('controle').Asinteger;
...
begin
  if qryitenslote.isempty then
    ...
    begin
      ...
      showmessage('Não existem registro a excluir!')
  else
      showmessage('Existem estagiários já selecionados!');
...
procedure TFORLOTE2.PgprincipalChange(Sender: TObject);
var
  edicao: boolean;
begin
  edicao := (Pgprincipal.ActivePage = tbRegistro);

  btNovo.Enabled    := not edicao;
  btExcluir.Enabled := (not edicao) and (not quPrincipal1.IsEmpty);

  btCancelar.Enabled := edicao;
  btSalvar.Enabled := edicao;

  btEmail.Enabled := not edicao;
  SpeedButton1.Enabled := not edicao;
end;

procedure TFORLOTE2.DBGrid4DblClick(Sender: TObject);
begin
  if not quPrincipal1.isEmpty then
  begin;
    pgPrincipal.ActivePage := tbRegistro;
    PgprincipalChange(nil);
  end;
