...
begin
  // Calcula o mês, ano e semestre atual
...
begin
  ...
  begin
    frmatualizando.Label1.caption := 'Aguarde Atualizando Alunos do Ensino Médio';
 ...
 //   showmessage('pare');
  ...
  // Verificando alunos do Técnico
  // and (Tce.Tce_situacao < 7)
  //'  Estudante.est_cod = Tce.Est_cod and ' +
  linha2 := ' Estudante.cur_cod = Curso.cur_cod  and pcu_anoatual is not null and';
  linha2 := linha2 + '  cur_nivel  = ''Técnico''  and est_cursoano = ' + ano;
  dmdta.qrypesquisa.SQL.Text := linha + ' where ' + linha2 + ' ORDER BY Curso.cur_nivel, Estudante.Est_nome ';
  dmdta.qrypesquisa.open;
  if dmdta.qrypesquisa.EOf = false then
  begin
    frmatualizando.Label1.caption := 'Aguarde Atualizando Alunos do Curso Técnico';
    dmdta.qrypesquisa.last;
    frmatualizando.progresso.Maxvalue := dmdta.qrypesquisa.RowsAffected;
    frmatualizando.progresso.progress := 0;
    frmatualizando.Show;
  ...
  //(Tce.Tce_situacao < 7)  and
    ...
    linha2 := linha2 + '  cur_nivel  not in (''Médio'',''Técnico'') and est_cursoano = ' + ano;
    dmdta.qrypesquisa.SQL.Text := linha + ' where ' + linha2 + ' ORDER BY Curso.cur_nivel, Estudante.Est_nome ';
    dmdta.qrypesquisa.open;
    if dmdta.qrypesquisa.EOf = false then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Var1.Bold;
      //var1.Insert(#13 + 'Mês: ' + Copy('00' + IntToStr(FORSEL1.mes), length(IntToStr(FORSEL1.mes)) + 1, 2) + '/' + IntToStr(FORSEL1.ano) + #13 + #13);
      ...
      Var1.Insert(#13);
      Var1.Insert(#9 + #9 + '                Total de estagiários: ' + IntToStr(RecordCount));
      Close;
    end;
...
begin
  ...
  if InputQuery('Data Inicial', 'Informe a data de referência:', DataIni) then
  begin
    try
      DataI := StrToDate(DataIni);
    except
      MSGERRO('Parâmetro errado. Tente novamente!');
      Exit;
    end;
      ...
      Sql.Clear;
{
    ...
    Rewrite(F);
    WriteLn(F, 'CETEFE  -  Estagiários a Rescindir');
    WriteLn(F, 'Data: ' + DateToStr(Data));
    WriteLn(F);
    WriteLn(F, PreencheEspacosEsquerda('TCE', 9) + ' ' + PreencheEspacosDireita('Nome', 50) + ' ' + PreencheEspacosDireita('Empresa', 100) + ' ' + PreencheEspacosDireita('Início', 13) +
...
begin
  ...
  begin
    ...
    // empresa
    InputQuery('Empresa', 'Informe a empresa de início:', sNomeEmpresa);
...
//      Sql.Add('a.tpr_datafim between ' + DataCerta + ' and ' + DataCerta);
      ...
      begin
        Screen.Cursor := crDefault;
        MSGERRO('Não há estudantes a serem rescindidos no dia: ' + DateToStr(Data) + '!');
        Exit;
      end;
      ...
      // posiciona no nome da empresa caso usuário tenha selecionado
      ...
      while not Eof do
      begin
        // Mostra a tela com os dados do estagiário
        ...
        begin
          ...
          begin
            // testa se não chegou nos 24 meses.
        ...
        sAssunto := 'Vencimento do Termo de Compromisso de Estágio - ' +
          FieldByName('est_nome').Text;

        // manda e-mail para o estudante
        FOREML := TFOREML.Create(Self);
        FOREML.RegistrarEmail(sAssunto, sPara, sCopiaOculta, sCorpo, nil, 'E-Mail do Responsável pela Empresa:');
        Screen.Cursor := crDefault;
        FOREML.ShowModal;
...
begin
  ...
  begin
    ...
    GerarExcel(dmDta.quAnivInstituicao, 'Func. Instituição Aniversariantes', 'Dia ' + DataIni);
    Screen.Cursor := crHourGlass;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Eof then
      begin
        MSGAVISO('Não há empresas cadastradas no dia ' + DataIni);
        Exit;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Eof then
      begin
        MSGAVISO('Não há TCE´s emitidos no dia ' + DataIni);
        Exit;
      end;
      ...
      begin
        ...
        Var1.Insert('Data de Início do TCE:');
        Var1.NextCell;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Eof then
      begin
        MSGAVISO('Não há TCE´s rescindidos no dia ' + DataIni);
        Exit;
      end;
...
begin
  ...
  begin
    ...
    Open;
    GerarExcel(DmDta.quEMailInstituicao, 'E-Mails das Instituições de Ensino');
    Close;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ExportarDadosExcel(dmdta.qrytces, campos, '', 'Tces', 'Período ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + ' - ' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date));
      close;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      if forsel2.outros.Checked then
        Sql.Add('where  (est_banco <> ''Banco Itaú'' or est_banco is null) and')
      else
        Sql.Add('where  est_banco = ''Banco Itaú'' and');
      Sql.Add('  est_cod in');
      Sql.Add('(select');
      Sql.Add('  est_cod');
      Sql.Add(' from ');
     ...
     // var1.Insert(#13 + 'Periodo: '+ Data1 +' .. '+data2+ #13 + #13);
      ...
      Var1.Insert(#13);
      Var1.Insert(#9 + #9 + '                Total de estagiários: ' + IntToStr(RecordCount) + #9);
...
begin
  ...
  begin
    ...
    begin
      ...
      campos.Items.Clear;
      if forsel2.outros.Checked then
        ExportarDadosExcel(DmDta.qryitau, campos, '', 'Relatório Geral Outros Bancos', 'Período ' + formatdatetime('dd/mm/yyyy', forsel2.Data) + ' - ' + formatdatetime('dd/mm/yyyy', forsel2.Datafim))
      else
        ExportarDadosExcel(DmDta.qryitau, campos, '', 'Relatório Geral Itaú', 'Período ' + formatdatetime('dd/mm/yyyy', forsel2.Data) + ' - ' + formatdatetime('dd/mm/yyyy', forsel2.Datafim));
//      GerarExcel(DmDta.qryitau, 'Relatório Geral Itaú');
      Close;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      campos.Items.Clear;
      if forsel3.bancos.itemindex = 0 then
        ExportarDadosExcel(DmDta.qryitau, campos, '', 'Relatório Geral (Cetefe)', 'Período ' + formatdatetime('dd/mm/yyyy', forsel3.Data) + ' - ' + formatdatetime('dd/mm/yyyy', forsel3.Datafim));
      if forsel3.bancos.itemindex = 2 then
        ExportarDadosExcel(DmDta.qryitau, campos, '', 'Relatório Geral Outros Bancos (Cetefe)', 'Período ' + formatdatetime('dd/mm/yyyy', forsel3.Data) + ' - ' + formatdatetime('dd/mm/yyyy', forsel3.Datafim));
      if forsel3.bancos.itemindex = 1 then
        ExportarDadosExcel(DmDta.qryitau, campos, '', 'Relatório Geral Itaú (Cetefe)', 'Período ' + formatdatetime('dd/mm/yyyy', forsel3.Data) + ' - ' + formatdatetime('dd/mm/yyyy', forsel3.Datafim));
...
begin
  ...
  begin
    ...
    qryitenslote.open;
    campos.Items.Clear;
    ExportarDadosExcel(qryitenslote, campos, '', 'Relatório Pagamentos Efetuados (Cetefe)', 'Período ' + forsel1.edano.Text + ' - ' + forsel1.edmes.Text);
    qryitenslote.close;
  end;
...
begin
  ...
  begin
    ...
    qryne.Open;
    campos.Items.Clear;
    ExportarDadosExcel(qryne, campos, '', 'Relatório Empresas Sem Fatura', 'Período ' + IntToStr(FORSEL1.Mes) + '/' + IntToStr(FORSEL1.Ano));
    qryne.close;
  end;
  FORSEL1.Free;
end;
...
begin
  ...
  begin
    ...
    if qryitenslotecod_retorno.AsString = 'AG' then
      qryitensloteRetorno.AsString := 'NUMERO DE LOTE INVÁLIDO';
    if qryitenslotecod_retorno.AsString = 'AH' then
      qryitensloteRetorno.AsString := 'NÚMERO SEQUENCIAL DE RESTRO NO LOTE INVÁLIDO';
    if qryitenslotecod_retorno.AsString = 'AI' then
      qryitensloteRetorno.AsString := 'PRODUTO DEMOSTRATIVO DE PAGAMENTO NÃO CONTRATADO';
    if qryitenslotecod_retorno.AsString = 'AJ' then
      qryitensloteRetorno.AsString := 'TIPO DE MOVIMENTO INVÁLIDO';
    if qryitenslotecod_retorno.AsString = 'AL' then
      qryitensloteRetorno.AsString := 'CÓDIGO DO BANCO FAVORECIDO INVÁLIDO';
    if qryitenslotecod_retorno.AsString = 'AM' then
      qryitensloteRetorno.AsString := 'AGÊNCIA DO FAVORECIDO INVÁLIDA';
    if qryitenslotecod_retorno.AsString = 'AN' then
      qryitensloteRetorno.AsString := 'CONTA CORRENTE DO FAVORECIDO INVÁLIDA';
    if qryitenslotecod_retorno.AsString = 'AO' then
      qryitensloteRetorno.AsString := 'NOME DO FAVORECIDO INVÁLIDO';
    if qryitenslotecod_retorno.AsString = 'AP' then
      qryitensloteRetorno.AsString := 'DATA DE PAGAMENO/DATA DE VALIDADE/ HORA DO LANCAMENTO ARRECADAÇÃO / APURAÇÃO INVÁLIDA';
...
begin
  ...
  begin
    ...
    inst.Open;
    campos.Items.Clear;
    ExportarDadosExcel(inst, campos, '', 'Relatório Instituicões de Ensino', forsel4.edCliente.Text);
    inst.close;
  end;
  FORSEL4.Free;
end;
...
begin
  ...
  //  DatabaseName := DATABASE_NAME;
    ...
    Open;
    GerarExcel(qrypesquisa, 'Alunos que Não entregaram a Declaração de Matricula');
    Close;
  end;
...
begin
  ...
  //  DatabaseName := DATABASE_NAME;
    ...
    Sql.Text := memo4.Text;
    Open;
    GerarExcel(qrypendentes, 'Alunos Em débito com relatórios');
    Close;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            qrypendentesstatus1.AsString := qrypendentesstatus1.AsString + 'com Pendências';
            inc(pendencias);
        ...
        begin
          ...
          begin
            qrypendentesstatussup1.AsString := qrypendentesstatussup1.AsString + 'com Pendências';
            inc(pendencias);
      ...
      begin
        ...
        begin
          ...
          if fieldbyname('est_pendente').asstring = 'S' then
          begin
            qrypendentesstatus2.AsString := qrypendentesstatus2.AsString + 'com Pendências';
            inc(pendencias);
        ...
        begin
          ...
          if fieldbyname('sup_pendente').asstring = 'S' then
          begin
            qrypendentesstatussup2.AsString := qrypendentesstatussup2.AsString + 'com Pendências';
            inc(pendencias);
          end;
        end
        else
      ...
      begin
        ...
        begin
          ...
          if fieldbyname('est_pendente').asstring = 'S' then
          begin
            qrypendentesstatus3.AsString := qrypendentesstatus3.AsString + 'com Pendências';
            inc(pendencias);
          end;
        end
        else
        ...
        begin
          ...
          if fieldbyname('sup_pendente').asstring = 'S' then
          begin
            qrypendentesstatussup3.AsString := qrypendentesstatussup3.AsString + 'com Pendências';
            inc(pendencias);
          end;
        end
        else
      ...
      begin
        ...
        begin
          ...
          if fieldbyname('est_pendente').asstring = 'S' then
          begin
            qrypendentesstatus4.AsString := qrypendentesstatus4.AsString + 'com Pendências';
            inc(pendencias);
          end;
        end
        else
        ...
        begin
          ...
          if fieldbyname('sup_pendente').asstring = 'S' then
          begin
            qrypendentesstatussup4.AsString := qrypendentesstatussup4.AsString + ' com Pendências';
            inc(pendencias);
          end;
        end
        else
...
begin
  ...
  begin
    ...
    begin
      ...
      Excel.Cells[1, 1] := 'NÚMERO DE ESTAGIÁRIOS - CETEFE';
      ...
      Excel.Cells[2, 1] := 'Período: ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + ' - ' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date);
      linha := 3;
      coluna := 1;
...
begin
  ...
  begin
    ...
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Relação de Contatos Por Empresa', 'Intervalo ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + '..' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date));
    qrypesquisa.close;
  end;
  frmintervalo.filtro.Visible := true;
...
begin
  ...
  begin
    ...
    qrypesquisa.parambyname('final').asstring := formatdatetime('dd/mm/yyyy', frmintervalo.final.date) + ' 23:59:29';
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Relação de Contatos Por Estudantes', 'Intervalo ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + '..' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date));
    qrypesquisa.close;
  end;
  frmintervalo.filtro.Visible := true;
...
begin
  ...
  begin
    ...
    qrypesquisa.parambyname('final').asstring := formatdatetime('dd/mm/yyyy', frmintervalo.final.date) + ' 23:59:29';
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Relação de Contatos Por Funcionário', 'Intervalo ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + '..' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date));
    qrypesquisa.close;
  end;
  frmintervalo.filtro.Visible := true;
...
begin
  ...
  begin
    ...
    qrypesquisa.parambyname('final').asstring := formatdatetime('dd/mm/yyyy', frmintervalo.final.date) + ' 23:59:29';
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Relação de Contatos Por Vaga', 'Intervalo ' + formatdatetime('dd/mm/yyyy', frmintervalo.inicio.date) + '..' + formatdatetime('dd/mm/yyyy', frmintervalo.final.date));
    qrypesquisa.close;
  end;
  frmintervalo.filtro.Visible := true;
...
begin
  ...
  qrypesquisa.SQL.Text := 'select EST_COD,EST_NOME ,CUR_COD,EST_CURSOANO,PCU_ANOATUAL,PCU_PERATUAL, (select cur_nivel from curso where cur_cod = estudante.cur_cod ) curso, ' +
    ' CASE when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Médio'' then 3 ' +
    ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Superior'' then 5 ' +
    ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Técnico'' then 4 ' +
    ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Pós-Médio'' then 2 ' +
    ' END TOTAL ' +
  ...
  if MSGSIMNAO('Confirme Processamento de Atualização de Periodo dos alunos do ano de ' + inttostr(ano - 1) + ' para o ano de ' + inttostr(ano)) = true then
  begin
    qrypesquisa.open;
    qrypesquisa.First;
    contador := 0;
    ...
    begin
      ...
      atualiza.SQL.Clear;
      tempo := '';
      if (trim(qrypesquisa.FieldByName('curso').AsString) = 'Médio') or (length(trim(qrypesquisa.FieldByName('curso').AsString)) = 5) then
        tempo := '3';
      if (trim(qrypesquisa.FieldByName('curso').AsString) = 'Superior') or (length(trim(qrypesquisa.FieldByName('curso').AsString)) = 8) then
        tempo := '5';
      if (trim(qrypesquisa.FieldByName('curso').AsString) = 'Técnico') or (length(trim(qrypesquisa.FieldByName('curso').AsString)) = 7) then
        tempo := '4';
      if (trim(qrypesquisa.FieldByName('curso').AsString) = 'Pós-Médio') or (length(trim(qrypesquisa.FieldByName('curso').AsString)) = 9) then
        tempo := '2';
      if (trim(qrypesquisa.FieldByName('curso').AsString) = 'Pós-Graduação') or (length(trim(qrypesquisa.FieldByName('curso').AsString)) = 13) then
        tempo := '2';
    ...
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Estudantes não Atualizado (Anual)');

    // por semestre
    qrypesquisa.Close;
    Qrypesquisa.SQL.Clear;
    qrypesquisa.SQL.Text := 'select EST_COD,EST_NOME,CUR_COD,EST_CURSOANO,PCU_ANOATUAL,PCU_PERATUAL,(select cur_nivel from curso where cur_cod = estudante.cur_cod ) curso ,' +
      ' CASE when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Médio'' then 3*2 ' +
      ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Superior'' then 5*2 ' +
      ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Técnico'' then 4*2 ' +
      ' when (select cur_nivel from curso where cur_cod = estudante.cur_cod )  = ''Pós-Médio'' then 2*2 ' +
      ' END TOTAL ' +
      ' from estudante where est_formado is null and est_trancado is null ' +
      ' and est_cursoano = ' + inttostr(ano - 1) + ' and est_cursosituacao in (''CURSANDO'',''FALTAM DADOS'') ' +
    ...
    qrypesquisa.Open;
    GerarExcel(qrypesquisa, 'Estudantes não Atualizado (Semestral)');

    //
    progresso.Visible := false;
    MSGAVISO('Processo de Atualização Finalizado! ' + inttostr(processados) + ' registros processados ' + inttostr(Atualizados) + ' Estudantes Atualizados!');
  end;
...
begin
  ...
  if mes < 7 then
  begin
    MSGAVISO('Não é possivel atualizar o Segundo Semestre antes do Mes de Julho!');
    exit;
  end;
  qrypesquisa.Close;
  qrypesquisa.SQL.Clear;
  ...
  if MSGSIMNAO('Confirme Processamento de Atualização de Periodo dos alunos do 1º Semestre de ' + inttostr(ano) + ' para o 2º Semestre do ano de ' + inttostr(ano)) = true then
  begin
    // por semestre
    qrypesquisa.Close;
    Qrypesquisa.SQL.Clear;
