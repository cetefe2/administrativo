...
var
  ...
  begin
    ...
    begin
      MessageBox(Handle, 'Defina a forma de pagamento (botão "Pagamento"->Forma de pagamento), antes de continuar!', 'Aviso', MB_OK or MB_ICONEXCLAMATION);
      exit;
    end;
    ...
    case DmDta.quEmpresaemp_tipotaxa.asinteger of
      ...
      3: begin
          ...
          tempo2 := '(' + extenso2(DmDta.quEmpresa.FieldByName('emp_valortaxa').ascurrency) + ') por estagiário';
        end;
      4: begin
          tempo := '';
          tempo0 := '';
...
var
  ...
  begin
    ...
    adicionaAviso(endEmp = '', 'O "Endereço" não foi definido.'); 
    ...
    adicionaAviso(cidadeemp = '', 'A "Cidade" não foi definida.'); 

    //var1.editbookmark('cepempresa', 0, 0, 0, 1);
    //Var1.Insert(CepEmp);
    var1.editbookmark('estadoempresa', 0, 0, 0, 1);
    Var1.Insert(estadoemp);
    adicionaAviso(cidadeemp = '', 'O "Estado" não foi definido.'); 

    var1.editbookmark('cnpjempresa', 0, 0, 0, 1);
    Var1.Insert(cnpjemp);
    adicionaAviso(cnpjemp = '', 'O "CNPJ" não foi definido.'); 
...
begin
  ...
  begin
      ...
      begin
        ...
        adicionaAviso(true, 'O funcionário responsável pelo contrato não foi encontrado!');
      end
      else
      begin
       // respemp := qrypesquisa.fieldbyname('Func_cod').asstring;
        ...
        adicionaAviso(caremp = '', 'O funcionário "' + respemp+ '" não tem o cargo preenchido!');
        adicionaAviso(cpfrespemp = '', 'O funcionário "' + respemp+ '" não tem o cpf preenchido!');
        adicionaAviso(rgrespemp = '', 'O funcionário "' + respemp+ '" não tem o rg preenchido!');

        adicionaAviso(nacionalidaderesp = '', 'O funcionário "' + respemp+ '" não tem a Nacionalidade preenchida!');
        adicionaAviso(estcivilresp = '', 'O funcionário "' + respemp+ '" não tem o Estado Civil preenchido!');
        adicionaAviso(profissaoresp = '', 'O funcionário "' + respemp+ '" não tem a Profissão preenchida!');
